...
const AirBadgeTable = () => {
  ...
  const [showMobileToggleState, setShowMobileToggleState] = useState(Boolean(window.matchMedia('(max-width: 768px)').matches))
  ...
  const [columnsSelectorsState, columnsSelectorsDispatch] = useReducer(columnsSelectorsReducer, initialColumnsSelectorsState)
    ...
    newState.data = newState.data.filter(data => {
      ...
      for (let i = 0; i < filterKeys.length; i++) {
        if (newState.filters[filterKeys[i]]) {
          if (toReturn) toReturn = data[filterKeys[i]].toLowerCase().match(newState.filters[filterKeys[i]].toLowerCase())
        }
      }
      return toReturn
    })
          ...
          <select
            ...
            onChange={event => {
              badgeDataDispatch({
                type: 'filter',
                target: columnName,
                value: filterTypes[columnName] === 'date select' ? new Date(event.target.value).toLocaleDateString() : event.target.value
              })
            }}
          >
    ...
    if (columnsSelectorsState[columnName]) {
      ...
      return (
        <Fragment key={columnName}>
          ...
          >
            {columnName}
            {filtersToggleState ? <div className="columnFilter" key={columnName}>{getFilter(columnName)}
            </div> : null}
          </div>
          ...
          {badgeDataState.data.map(data => {
            return (
              ...
              >
                ...
                <DataPopup
                  data={{ columnName, dataValue: data[columnName] }}
                ></DataPopup>
      ...
      >
        ...
        >
          ...
          >
            ...
            <div id="columnSelectorsSection" style={{ fontWeight: 400, fontSize: '18px', color: '#003852', margin: '1.5vmin 0', border: '2px solid #003852' }}>
              ...
              >
                ...
                <div
                  ...
                  style={{ ...columnSelectorsStyles, color: columnsSelectorsState['Card Number'] ? '#00618F' : 'black' }}
                >
                  Card Number
                </div>
                <div
